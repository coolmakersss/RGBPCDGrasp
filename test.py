...
def inference():
    test_dataset = GraspNetDataset_fusion(cfgs.dataset_root, valid_obj_idxs=None, grasp_labels=None, split='test_novel', camera=cfgs.camera,
                               num_points=cfgs.num_point, remove_outlier=True, augment=False, load_label=False, use_fine=False, exfeature='agg_dinov2')
    ...
    for batch_idx, batch_data in enumerate(test_dataloader):
        for key in batch_data:
            if 'list' in key:
                ...
                batch_data[key] = batch_data[key].float().to(device)

        #print(type(batch_data[key]))
        ...
        for i in range(cfgs.batch_size):
            ...
            save_path = os.path.join(save_dir, '0000.npy')
...
def evaluate(dump_dir):
    sceneIds=[i for i in range(160,187)]
    ge = GraspNetEval(root=cfgs.dataset_root, camera=cfgs.camera, split='custom', sceneIds=sceneIds)
    #ge = GraspNetEval(root=cfgs.dataset_root, camera=cfgs.camera, split='test_similar')
    res, ap = ge.eval_novel_modify(dump_folder=dump_dir, proc=6)
