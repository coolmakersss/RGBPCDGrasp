    ...
    def __init__(self, root, valid_obj_idxs, grasp_labels, camera='kinect', split='train', num_points=20000,
        ...
        for x in tqdm(self.sceneIds, desc='Loading data path and collision labels...'):
            if use_fine:
                ...
                self.pcdpath.append(os.path.join(root, 'fusion_scenes_dinov2_depth', x, camera, 'points.npy'))
                self.labelpath.append(os.path.join(root, 'fusion_scenes_dinov2_depth', x, camera, 'seg.npy'))
                self.graspnesspath.append(os.path.join(root, 'graspness_dinov2_depth', x, camera, 'graspness.npy'))
                self.featurepath.append(os.path.join(root, 'fusion_scenes_dinov2_depth', x, camera, 'dinov2_feature.npy'))
            elif self.exfeature == 'agg_dinov2':
                self.pcdpath.append(os.path.join(root, 'fusion_scenes_dinov2_depth', x, camera, 'points.npy'))
                self.labelpath.append(os.path.join(root, 'fusion_scenes_dinov2_depth', x, camera, 'seg.npy'))
                self.graspnesspath.append(os.path.join(root, 'graspness_dinov2_depth', x, camera, 'graspness.npy'))
                self.featurepath.append(os.path.join(root, 'fusion_scenes_dinov2_depth', x, camera, 'agg_feature.npy'))
