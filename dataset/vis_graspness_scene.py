...
fusion_data = np.load(os.path.join(dataset_root, 'fusion_scenes_dinov2_depth', 'scene_' + str(scene_id).zfill(4),
                                            camera_type, 'points.npy'),allow_pickle=True).item()
seg = np.load(os.path.join(dataset_root, 'fusion_scenes_dinov2_depth', 'scene_' + str(scene_id).zfill(4), camera_type, 'seg.npy'))
point_cloud = np.array(fusion_data['xyz'])
color = np.ones_like(point_cloud)

graspness_full = np.load(os.path.join(dataset_root, 'graspness_dinov2_depth', 'scene_' + str(scene_id).zfill(4), camera_type, 'graspness.npy')).squeeze()
graspness_full[seg == 0] = 0.
print('graspness full scene: ', graspness_full.shape, (graspness_full > 0.09).sum())
color[graspness_full > 0.09] = [0., 1., 0.]
#color[seg > 0] = [0., 1., 0.]
