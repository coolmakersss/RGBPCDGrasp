...
from models.modules import ApproachNet, GraspableNet, CloudCrop, SWADNet, MLP
from loss_utils import GRASP_MAX_WIDTH, NUM_VIEW, NUM_ANGLE, NUM_DEPTH, GRASPNESS_THRESHOLD, M_POINT, FEATURE_DIM
...
class GraspNet(nn.Module):
    def __init__(self, cylinder_radius=0.05, seed_feat_dim=512, is_training=True):
        ...
        self.seed_feature_dim_fusion = seed_feat_dim + FEATURE_DIM
        ...
        self.mapping = MLP(input_size=self.seed_feature_dim_fusion, hidden_size=512, output_size=self.seed_feature_dim)
    ...
    def forward(self, end_points):
        ...
        #print(end_points['quantize2original'])
        seed_features = seed_features[end_points['quantize2original'].long()].view(B, point_num, -1)

        '''
        seed_features = torch.cat((seed_features, end_points["exfeature"]),dim=2)
        print(seed_features.shape)
        seed_features = self.mapping(seed_features)
        ...
        print("objectness_mask: ", objectness_mask.sum())
        print("graspness_mask: ", graspness_mask.sum())
        print("graspable_mask: ",graspable_mask.sum())
        cloud = o3d.geometry.PointCloud()
        xyz = seed_xyz[0].cpu()
        cloud.points = o3d.utility.Vector3dVector(np.array(xyz).astype(np.float32))
        #color = np.array(end_points['color'][0].cpu())
        #print(color)
        color = np.tile(np.array([255., 255., 255.]), (point_num, 1))
        color[objectness_mask[0].cpu()] = [0., 0., 255.]
        color[graspness_mask[0].cpu()] = [0., 255., 0.]
        color[graspable_mask[0].cpu()] = [255., 0., 0.]
        cloud.colors = o3d.utility.Vector3dVector(color.astype(np.float32) / 255.)
        #o3d.visualization.draw_geometries([cloud])
        o3d.io.write_point_cloud("graspable_point.ply", cloud)
        ...
        for i in range(B):
            ...
            #print(seed_features_flipped.shape,' cur_mask:',cur_mask)
            ...
            #print("cur_seed_xyz_flipped: ", cur_seed_xyz_flipped.shape, cur_seed_xyz_flipped)
            ...
            #print("cur_feat_flipped: ", cur_feat_flipped.shape, cur_feat_flipped)
            cur_feat = gather_operation(cur_feat_flipped, fps_idxs).squeeze(0).contiguous() # feat_dim*Ns
            #print("cur_feat: ",cur_feat.shape)
            seed_features_graspable.append(cur_feat)
            seed_xyz_graspable.append(cur_seed_xyz)

        cloud = o3d.geometry.PointCloud()
        xyz = seed_xyz_graspable[0].cpu()
        cloud.points = o3d.utility.Vector3dVector(np.array(xyz).astype(np.float32))
        #o3d.visualization.draw_geometries([cloud])
        o3d.io.write_point_cloud("graspable_point_1024.ply", cloud)
        ...
        #print(seed_features_graspable.shape)
        ...
        #print(end_points['grasp_top_view_rot'].shape)
        #print(end_points['grasp_top_view_rot'])
        #print(end_points['grasp_top_view_xyz'].shape)
        #print(end_points['grasp_top_view_xyz'])
...
class GraspNet_rgb(nn.Module):
    def __init__(self, cylinder_radius=0.05, seed_feat_dim=512, is_training=True):
        super().__init__()
        self.is_training = is_training
        self.seed_feature_dim = FEATURE_DIM
        self.seed_feature_dim_fusion = seed_feat_dim + FEATURE_DIM
        self.num_depth = NUM_DEPTH
        self.num_angle = NUM_ANGLE
        self.M_points = M_POINT
    ...
    def forward(self, end_points):
        ...
        #mink_input = ME.SparseTensor(features_batch, coordinates=coordinates_batch)
        #seed_features = self.backbone(mink_input).F
        #seed_features = seed_features[end_points['quantize2original']].view(B, point_num, -1)

        
        seed_features = end_points["exfeature"]
        print(seed_features.shape)
        #seed_features = self.mapping(seed_features)
        seed_features = seed_features.transpose(1,2)
        print(seed_features.shape)
