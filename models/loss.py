...
def get_loss(end_points):
    ...
    #loss = objectness_loss + 20 * graspness_loss + 100 * view_loss + 20 * score_loss + 10 * width_loss
...
def compute_graspness_loss(end_points):
    ...
    rank_error = (torch.abs(torch.trunc((graspness_score_c * 20).float()) - torch.trunc((graspness_label_c * 20).float())) / 20.).mean()
    end_points['stage1_graspness_acc_rank_error'] = rank_error.float()

    end_points['loss/stage1_graspness_loss'] = loss.float()
    return loss, end_points
...
def compute_width_loss(end_points):
    ...
    #print(grasp_width_pred," grasp_width_pred: ",grasp_width_pred.shape)
    #print(grasp_width_label," grasp_width_label: ",grasp_width_label.shape)
    loss = criterion(grasp_width_pred, grasp_width_label)
    #print("width_loss: ",loss)
    #print("loss_has_nan: ",torch.isnan(loss).any().item())
    grasp_score_label = end_points['batch_grasp_score']
    #print(grasp_score_label," ", grasp_score_label.shape, " ",grasp_score_label.sum())
    loss_mask = grasp_score_label > 0
    #print(loss_mask.sum(),"loss_mask: ",loss_mask.shape)
