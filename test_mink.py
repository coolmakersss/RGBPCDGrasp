...
# from graspnet_denoise import GraspNet_Denoise, pred_decode
from graspnet_sparseconv import GraspNet_MSCQ,GraspNet_MSCQ_approach_sample, GraspQNet,pred_decode,pred_decode_reg
# from graspnet_mink_onestage import GraspNet_singlestage,pred_decode_reg
...
# TEST_DATASET = GraspNetDataset_complete(cfgs.dataset_root, valid_obj_idxs=None, grasp_labels=None, split='test', camera=cfgs.camera,
...
# TEST_DATASET = GraspNetDataset_complete(cfgs.dataset_root, valid_obj_idxs, grasp_labels, camera=cfgs.camera, split='test',
...
net = GraspNet_MSCQ(input_feature_dim=0, num_view=cfgs.num_view, num_angle=12, num_depth=4,
                    cylinder_radius=0.08, hmin=-0.02, hmax_list=[0.01,0.02,0.03,0.04], is_training=False)
# net = GraspNet_MSCQ_approach_sample(input_feature_dim=0, num_view=cfgs.num_view, num_angle=12, num_depth=4,
#                      cylinder_radius=0.08, hmin=-0.02, hmax_list=[0.01,0.02,0.03,0.04], is_training=False)
...
def inference():
    ...
    for batch_idx, batch_data in enumerate(TEST_DATALOADER):
        ...
        with torch.no_grad():
            ...
            # grasp_preds, save_preds = pred_decode_reg(end_points)
