...
from models.graspnet import GraspNet, GraspNet_rgb
from models.loss import get_loss
from dataset.graspnet_dataset import GraspNetDataset_fusion, minkowski_collate_fn, load_grasp_labels
...
parser.add_argument('--max_epoch', type=int, default=400, help='Epoch to run [default: 18]')
...
TRAIN_DATASET = GraspNetDataset_fusion(cfgs.dataset_root, valid_obj_idxs, grasp_labels=grasp_labels, camera=cfgs.camera, split='train',
                                num_points=cfgs.num_point, voxel_size=cfgs.voxel_size,
                                remove_outlier=True, augment=True, load_label=True, use_fine=False, exfeature='dinov2')
...
#net = GraspNet_rgb(seed_feat_dim=cfgs.seed_feat_dim, is_training=True)
...
#resume train
scheduler = OneCycleLR(optimizer, max_lr=cfgs.learning_rate,div_factor=10.0, final_div_factor=100.0, steps_per_epoch=len(TRAIN_DATALOADER),
