...
# from graspnet_dataset import GraspNetSegDataset, collate_fn, load_grasp_labels
# from graspnet_wonoise_dataset import GraspNetDataset_Align,GraspNetDataset_mix,GraspNetDataset_mix_inverse, collate_fn, load_grasp_labels
from mink_dataset import GraspNetDataset,GraspNetDataset_complete,GraspNetDataset_fusion, minkowski_collate_fn, load_grasp_labels
...
if cfgs.net == "base":
    ...
    from loss import get_loss as get_loss
    ...
    from graspnet_sparseconv import GraspNet_MSCQ
    from loss import get_loss as get_loss
    from loss import get_eval_loss
    net = GraspNet_MSCQ(input_feature_dim=0, num_view=cfgs.num_view, num_angle=12, num_depth=4,
                   cylinder_radius=0.08, hmin=-0.02, hmax_list=[0.01, 0.02, 0.03, 0.04])

elif cfgs.net == "view_pred":
    print(cfgs.net)
    from graspnet_sparseconv import GraspNet_MSCQ
    from loss import get_loss_viewpred as get_loss
    from loss import get_eval_loss
    net = GraspNet_MSCQ(input_feature_dim=0, num_view=cfgs.num_view, num_angle=12, num_depth=4,
                   cylinder_radius=0.08, hmin=-0.02, hmax_list=[0.01, 0.02, 0.03, 0.04])

elif cfgs.net == "one_stage":
    print(cfgs.net)
    from graspnet_mink_onestage import GraspNet_singlestage
    from loss import get_loss_single as get_loss
    net = GraspNet_singlestage(input_feature_dim=0, num_view=cfgs.num_view, num_angle=12, num_depth=4,
                   cylinder_radius=0.08, hmin=-0.02, hmax_list=[0.01, 0.02, 0.03, 0.04])
...
#freeze stage1
# params_list=[]
# params_list.append(dict(params=net.grasp_generator.parameters(), lr=cfgs.learning_rate))
# optimizer = optim.Adam(params_list, lr=cfgs.learning_rate, weight_decay=cfgs.weight_decay)
optimizer = optim.Adam(net.parameters(), lr=cfgs.learning_rate, weight_decay=cfgs.weight_decay)
# checkpoint = torch.load("/home/LAB/r-yanghongyu/graspnet/logs/log_complete_mink_base_objgraspness/checkpoint.tar")['model_state_dict']
...
if CHECKPOINT_PATH is not None and os.path.isfile(CHECKPOINT_PATH):
    ...
    # sup_set = checkpoint['sup_set']
    # print("loaded supervised set",sup_set)
    # unsup_set = checkpoint['unsup_set']
...
def train_one_epoch():
    ...
    for batch_idx, batch_data_label in enumerate(TRAIN_DATALOADER):
        ...
        if (batch_idx + 1) % 1 == 0:
            ...
            optimizer.zero_grad()
            scheduler.step()
        # print(net.view_estimator.backbone.conv0p1s1.kernel[0,0,:5])
...
def evaluate_one_epoch():
    ...
    for batch_idx, batch_data_label in enumerate(TEST_DATALOADER):
        ...
        # Compute loss
        end_points['epoch'] = EPOCH_CNT
        loss, end_points = get_eval_loss(end_points)
